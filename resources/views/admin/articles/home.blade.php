...
<div id="home_articles" >
    <div class="ibox-content">
        <div class="table-responsive">
            <div class="m-b-sm">
                <a href="{{ url('admin/articles/create')}}" type="button" class="btn btn-primary btn">Add new Post</a>
                @if(!$trash)
                <a href="{!! URL::route('articels.list_cat',['trash'=>1,'cat_id'=>$cat->id]) !!}" class="btn btn-warning btn demo1"><i class="glyphicon glyphicon-trash" aria-hidden="true"></i> View Trash</a>
                @else
                <a href="{!! URL::route('articels.list_cat',['cat_id'=>$cat->id]) !!}" class="btn btn-primary  "><i class="glyphicon glyphicon-check" aria-hidden="true"></i> View All</a>
                @endif
                ...
                <a href="{!! URL::route('articels.list_cat',['trash'=>$trash,'cat_id'=>$cat->id, 'lang' => 'en']) !!}" class="btn btn-warning btn demo1">English</a>
                <a href="{!! URL::route('articels.list_cat',['trash'=>$trash,'cat_id'=>$cat->id, 'lang' => 'vi']) !!}" class="btn btn-warning btn demo1">Tiếng Việt</a>
                <a href="{!! URL::route('articels.list_cat',['trash'=>$trash,'cat_id'=>$cat->id, 'lang' => 'all']) !!}" class="btn btn-warning btn demo1">Tất cả</a>


                <a href="{!! URL::route('tests.list',['cat_id'=>$cat->id]) !!}" type="button" class="btn btn-primary btn">Tests</a>

            </div>
            <form action="{!! URL::route('articels.deleteArts') !!}" action="" method="POST" id="delete_form">
                ...
                <table class="table">
                    ...
                    <tbody>
                        ...
                        <tr>	
                            ...
                            <td>{{$i++}}</td>
                            <td > {{$post->id}} </td>
                            <td > <a href="{{ url('admin/articles/edit/'. $post->id) }}" class="btn btn-primary">{{$post->title}}</a> </td>
                            <td > {!!$post->thumbnail ? "<img width='120' src='".$post->thumbnail."' />" : ""!!} </td>

                            <td > <a href="{{$post->link}}" target="_blank">Link</a> </td>
                            <td >{{ trim(strip_tags($post->excerpt)) ?  str_limit(strip_tags($post->excerpt),50, '...') : str_limit(strip_tags($post->content),100, '...')}}</td>
                            <td > {{$post->cat->name}} </td>
                            <td>
